...
function capitalizeWords(str) {
  ...
  const upperWords = wordsList.map((word) => capitalize(word)); // [Do], [All], ['The'], ['Things']
...
function kebobCase(str, separator = '-') {
  ...
  const filterdWords = splitChrs.filter((chr) => {
    ...
    if (code > 96 && code < 123) { // keep letters
      ...
      return true;
    } if (code === 32 || code === 45) { // keep hyphen
      return true;
    }
    return false;
  });
  ...
  // remove extra spaces
  const spaceFree = removeExtraSpaces(joinedWords);
...
function makeHashTag(str) {
  ...
  if (splitWords.length > 3) {
    const sortedArr = splitWords.sort((a, b) => b.length - a.length);
